...
func TestSnapshotIsDeterministic(t *testing.T) {
	...
	wantManifest := `{"schema":"sworn.protocol-assets/v1","commit":"` + commit +
...
func newRepository(t *testing.T) string {
	...
	repo := t.TempDir()
	git(t, repo, "init", "-q")
	git(t, repo, "config", "user.name", "Protocol Assets Test")
	git(t, repo, "config", "user.email", "protocol-assets@example.invalid")
	return repo
}

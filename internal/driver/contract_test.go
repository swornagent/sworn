...
func TestCanonicalOperationsBindExactRC3Package(t *testing.T) {
	t.Parallel()
	...
	if identity != (PackageIdentity{
		Version:        "1.0.0-rc.3",
		TagObject:      "34324784694696a38d951061c2313363b405c1e4",
		Commit:         "affaf16cc37f845b5dc43b22988d8b680ff1f212",
		ManifestSHA256: "sha256:b58b3c2f844c0c0b07195c3ef5af8c1819871f11b51d697e06207ad0d4b2ec9c",
	}) {
...
func TestRequestCodecIsStrictBoundedAndStable(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"unsafe integer":  {bytes.Replace(body, []byte(`"timeout_ms":60000`), []byte(`"timeout_ms":9007199254740992`), 1), "INVALID_LIMIT"},
	}
...
func TestRC3WorkspaceAndRepositoryPathBoundaries(t *testing.T) {
	t.Parallel()
...
func TestFourModelRolesRequireExplicitModelAndVerifierIsolation(t *testing.T) {
	t.Parallel()
	for _, role := range []Role{RolePlanner, RoleImplementer, RoleCaptain, RoleVerifier} {
		request := contractRequest(t, role, pointer("explicit-model"))
		request.Model = nil
		if err := ValidateRequest(request); !IsCode(err, "INVALID_MODEL") {
			t.Fatalf("%s nil model error = %v", role, err)
		}
	}
	merge := contractRequest(t, RoleMerge, nil)
	merge.Model = pointer("forbidden")
	if err := ValidateRequest(merge); !IsCode(err, "INVALID_MODEL") {
		t.Fatalf("Merge model error = %v", err)
	}
	verifier := contractRequest(t, RoleVerifier, pointer("explicit-model"))
	verifier.Workspace.Access = ReadWrite
	if err := ValidateRequest(verifier); !IsCode(err, "INVALID_VERIFIER") {
		t.Fatalf("writable Verifier error = %v", err)
	}
	verifier.Workspace.Access = ReadOnly
	verifier.FreshContext = false
	if err := ValidateRequest(verifier); !IsCode(err, "INVALID_VERIFIER") {
		t.Fatalf("non-fresh Verifier error = %v", err)
	}
}

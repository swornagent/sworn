...
const (
	...
	exactClaudeBinary = "/home/brad/snap/code/247/.local/share/claude/versions/2.1.208"
)
...
func TestExactNativeProfilesInspectDoctorAndCertifyWithoutSubstitution(t *testing.T) {
	for _, family := range []ProfileFamily{ProfileCodex, ProfileClaude} {
		...
		t.Run(string(family), func(t *testing.T) {
			...
			var probedRef, probedModel string
			adapterValue, err := NewNativeAdapter(
				config,
				func(context.Context, string) (string, error) {
					return "/not-used-by-readiness", nil
				},
				func(_ context.Context, gotRef, gotModel string) error {
					probedRef, probedModel = gotRef, gotModel
					return nil
				},
			)
			if err != nil {
			...
			if state, code := adapter.checkProfile(
				...
				profile,
				"exact-native-model",
			); state != ReadinessPass || code != "live_probe_passed" ||
				probedRef != ref || probedModel != "exact-native-model" {
				t.Fatalf(
					"certify = %s %s, probe=(%q,%q)",
					state,
					code,
					probedRef,
					probedModel,
...
func TestNativeCommandSurfacesAreExactAndCapabilityIsSingleSeam(t *testing.T) {
	for _, family := range []ProfileFamily{ProfileCodex, ProfileClaude} {
		...
		t.Run(string(family), func(t *testing.T) {
			...
			switch family {
			case ProfileCodex:
				if bytes.Count(environmentBody, capability) != 1 ||
					bytes.Contains(mcpBody, capability) ||
			...
			case ProfileClaude:
				if bytes.Contains(environmentBody, capability) ||
					...
					!containsArgumentSequence(
						arguments,
						claudeArguments(invocation.Selected.Model),
					) {
					t.Fatalf(
...
func TestNativeInitializationCaptureRejectsAmbientCapabilities(t *testing.T) {
	t.Run("Claude", func(t *testing.T) {
		...
		if err := state.accept(body); err != nil || !state.initialized {

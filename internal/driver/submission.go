...
func DecodeSubmission(body []byte) (Submission, error) {
	...
	root, err := decodeTyped(
		body,
		MaxSubmissionBytes,
		[]string{"schema_version", "invocation_id", "artifacts", "decision"},
		nil,
		&submission,
	)
	if err != nil {
	...
	for _, value := range artifactValues {
		if _, err := closedObject(value,
			[]string{"kind", "byte_count", "digest", "bytes"}, nil); err != nil {
			return Submission{}, err
		}
	}
	if root["decision"] != nil {
		if _, err := closedObject(root["decision"],
			[]string{"outcome", "evidence_bytes", "evidence_digest", "evidence"}, nil); err != nil {
			return Submission{}, err
		}
	}
	if err := ValidateSubmission(submission); err != nil {
...
func (kind ArtifactKind) valid() bool {
	return kind == ArtifactPlan || kind == ArtifactDesign || kind == ArtifactWorkProof ||
		kind == ArtifactWorkStatus || kind == ArtifactAssemblyStatus
}
...
func (outcome DecisionOutcome) valid() bool {
	return outcome == DecisionProceed || outcome == DecisionRevise ||
		outcome == DecisionEscalate || outcome == DecisionPass ||
		outcome == DecisionFail || outcome == DecisionBlocked
}

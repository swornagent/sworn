...
type ResultBinding struct {
	InvocationID, DriverID, DriverVersion string
	Model                                 *string
	BindModel                             bool
}
...
func DecodeDriverInfo(body []byte, expected DriverInfoBinding) (DriverInfo, error) {
	...
	if _, err := decodeTyped(
		body,
		4_096,
		[]string{"contract_version", "driver_id", "driver_version"},
		nil,
		&info,
	); err != nil {
...
func ValidateRequest(request Request) error {
	...
	if request.Role == RoleMerge {
		if request.Model != nil {
			return fail("INVALID_MODEL")
		}
	} else if request.Model == nil || validateText(*request.Model, 500, false) != nil {
		return fail("INVALID_MODEL")
	}

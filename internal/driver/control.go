...
type terminalArbiter struct {
	...
	outputMaximum, outputTotal     int64
	output                         []byte
	result                         *Result
	usage                          UsageReceipt
...
func newTerminalArbiter(invocation Invocation, server *submissionServer) *terminalArbiter {
	arbiter := &terminalArbiter{
		server:        server,
		outputMaximum: MaxResultEnvelopeBytes,
		binding: ResultBinding{
			InvocationID:   invocation.Request.InvocationID,
			AdapterID:      invocation.Selected.Adapter.ID,
			AdapterVersion: invocation.Selected.Adapter.Version,

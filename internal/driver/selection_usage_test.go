...
func TestRoleSelectionsResolveOneExactProviderAndModel(t *testing.T) {
	...
	for role, want := range expected {
		...
		if err := ValidateSelectedResult(got, result); err != nil {
			t.Fatal(err)
		}
	}
...
func TestUsageNormalizationPreservesZeroAndUnavailable(t *testing.T) {
	...
	if unavailable.InputTokens != nil || unavailable.OutputTokens != nil ||
	...
	if string(unavailableBody) != `{"input_tokens":null,"output_tokens":null,"cost_micro_units":null,"currency":null,"source":null}` {
	...
	if string(body) != `{"input_tokens":0,"output_tokens":0,"cost_micro_units":0,"currency":"USD","source":"provider_reported"}` {

...
// validateLeadReleaseLineage proves provenance, not merely ancestry. Every
...
func (s *Service) validateLeadReleaseLineage(
	ctx context.Context,
	...
	if err != nil || !delegation.Active {
		return runtimeFail("LEAD_RELEASE_LINEAGE_REFUSED", err)
	}
	...
	defer engine.Close()
	return validateLeadReleaseLineageWithEngine(engine, manifest, proposal, snapshot, delegation)
}

func validateLeadReleaseLineageWithEngine(
	engine *engine,
	manifest admittedManifest,
	proposal admittedPlanProposal,
	snapshot journal.Snapshot,
	...
	if stateErr != nil {
		if protocol.ErrorCode(stateErr) == "REF_NOT_FOUND" &&
			...
			if err := ValidateLeadPlanPolicy(delegation.Envelope.PlanRules, proposal.plan, nil); err != nil {
				return runtimeFail("LEAD_RELEASE_LINEAGE_REFUSED", err)
			}
			return nil
		}
		return runtimeFail("LEAD_RELEASE_LINEAGE_REFUSED", stateErr)
	}
	if proposal.authority.ReleaseHead != state.Refs.Release.Head ||
		...
		return runtimeFail("LEAD_RELEASE_LINEAGE_REFUSED", nil)
	}
	...
	if prior == nil || ValidateLeadPlanPolicy(delegation.Envelope.PlanRules, proposal.plan, prior) != nil {
		return runtimeFail("LEAD_RELEASE_LINEAGE_REFUSED", nil)
	}
	...
	proposals, err := allLeadPlanProposals(manifest, snapshot)
	if err != nil {
		return err
	}
	for index := start + 1; index < len(state.Plan.History); index++ {
		...
		if err := validateLeadInstalledProvenance(engine, manifest, snapshot, delegation, *installed, history.OID); err != nil {
			return err
		}
	}
	return nil
...
func allLeadPlanProposals(manifest admittedManifest, snapshot journal.Snapshot) ([]admittedPlanProposal, error) {
...
func validateLeadInstalledProvenance(
	engine *engine,
	manifest admittedManifest,
	snapshot journal.Snapshot,
	delegation LeadDelegationState,
	...
	for _, command := range snapshot.Commands {
		...
		if command.Kind == "lead_decision" {
			...
			canonical, canonicalErr := func() ([]byte, error) {
				...
				return CanonicalLeadDecisionCommand(decision)
			}()
			if canonicalErr == nil && bytes.Equal(canonical, command.Payload) &&
				...
				ok && effect.Kind == leadDecisionEffectKind && effect.State == journal.Succeeded {
				decisions++
			}
		}
	}
	...
	for _, effect := range snapshot.Effects {
		if effect.Kind != "protocol.install" || effect.State != journal.Succeeded || effect.BeforeDigest != work {
			continue
		}
		...
		result, validateErr := validateSucceededProtocolAction(engine, command, effect, persisted)
		if parseErr != nil || validateErr != nil || result.Plan != planOID {
			return runtimeFail("LEAD_RELEASE_LINEAGE_REFUSED", errors.Join(parseErr, validateErr))
		}
		installs++
	}

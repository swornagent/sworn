...
const trackBaseCommandVersion = "sworn.git-prepare-track-base/v1"
...
func trackBaseRequestForSlice(
	...
	request, err := trackBaseRequestFromWire(engine, wire)
	if err != nil {
...
func validateTrackBaseCommand(
	...
	request, err := trackBaseRequestFromWire(engine, persisted.Request)
	if err != nil {
		return trackBaseCommand{}, gitx.PrepareTrackBaseRequest{},
			runtimeFail("CORRUPT_JOURNAL", err)
	}
...
func (s *Service) prepareTrackBaseForSlice(
	...
	if slice == nil || len(slice.Location.Slice.Consumes) == 0 {

...
// TestProviderUnavailableFailureParksTheWorkBeforeTheNextTry pins #310: a
// single provider-refused try (PROVIDER_LIMITED) parks the work before the
// next try burns, the park event and status both name the cause and the
// provider's own message, a transport fault stays an ordinary try, and a
// later successful try retires the crossing.
func TestProviderUnavailableFailureParksTheWorkBeforeTheNextTry(t *testing.T) {
	t.Parallel()
	fixture := newEconomyGuardFixture(t, driver.Limits{
		TimeoutMillis: 30_000, OutputBytes: 65_536,
	})
	work := fixture.readyWorkForLane(t, "T1")
	runID := fixture.manifest.value.RunID
	usage := economyUsageReceipt(t, "sworn.openai", 10, 20, 1, 0)
	const detail = "exit status 1: error_during_execution: You've hit your usage limit"

	// A transport fault is a try, not a provider refusal.
	fixture.failedDispatchAttempt(t, work, 1, 1, "PROVIDER_TRANSPORT_FAILED", "", usage)
	parked, err := fixture.service.economyGuardsParked(fixture.ctx, fixture.manifest, runID, work)
	if err != nil {
		t.Fatal(err)
	}
	if parked {
		t.Fatal("a transport fault parked the work as provider unavailable")
	}

	fixture.failedDispatchAttempt(t, work, 1, 2, "PROVIDER_LIMITED", detail, usage)
	parked, err = fixture.service.economyGuardsParked(fixture.ctx, fixture.manifest, runID, work)
	if err != nil {
		t.Fatal(err)
	}
	if !parked {
		t.Fatal("a provider-limited try did not park the work before the next try")
	}
	snapshot, err := fixture.store.Snapshot(fixture.ctx, runID)
	if err != nil {
		t.Fatal(err)
	...
	if parsed.Cause != ParkCauseProviderUnavailable ||
		parsed.FailureCode != "PROVIDER_LIMITED" ||
		parsed.FailureDetail != detail ||
		parsed.Work != work || parsed.UnblockKnob != "" {
		t.Fatalf("park event = %#v", parsed)
	}
	...
	if status.State != "parked" || status.Park == nil ||
		status.Park.Cause != ParkCauseProviderUnavailable ||
		status.Park.FailureCode != "PROVIDER_LIMITED" ||
		status.Park.FailureDetail != detail ||
		status.Park.Work != work {
		t.Fatalf("status = %q park = %#v", status.State, status.Park)
	}
	if len(status.PinnedWork) != 1 ||
		status.PinnedWork[0].WorkID != work ||
		status.PinnedWork[0].Cause != ParkCauseProviderUnavailable ||
		status.PinnedWork[0].Code != "PROVIDER_LIMITED" {
		t.Fatalf("pinned work = %#v", status.PinnedWork)
	}

	// A later try that the provider accepted retires the crossing.
	fixture.contextualSucceededDispatchAttempt(
		t, work, 1, 3, "S1", driver.ImplementerImplementation, 1)
	snapshot, err = fixture.store.Snapshot(fixture.ctx, runID)
	if err != nil {
		t.Fatal(err)
	}
	...
	if crossings := providerUnavailableParkCrossings(snapshot, control); len(crossings) != 0 {
		t.Fatalf("crossing survived a later accepted try: %#v", crossings)
	}
}

// TestProviderUnavailableParkEventShape pins the canonical body: the work
// and the provider code are required, no knob, and every other counted
// field must be zero.
func TestProviderUnavailableParkEventShape(t *testing.T) {
	t.Parallel()
	work := "sha256:" + strings.Repeat("a", 64)
	valid := DegradationParkEvent{
		SchemaVersion: ParkEventVersion, RunID: "run-310",
		Cause: ParkCauseProviderUnavailable, FailureCode: "PROVIDER_LIMITED",
		FailureDetail: "usage limit", Work: work,
	}
	if _, err := canonicalDegradationParkEvent(valid); err != nil {
		t.Fatalf("valid body = %v", err)
	}
	for name, mutate := range map[string]func(*DegradationParkEvent){
		"missing work":    func(e *DegradationParkEvent) { e.Work = "" },
		"missing code":    func(e *DegradationParkEvent) { e.FailureCode = "" },
		"knob present":    func(e *DegradationParkEvent) { e.UnblockKnob = "x" },
		"consecutive set": func(e *DegradationParkEvent) { e.Consecutive = 1 },
		"reason present":  func(e *DegradationParkEvent) { e.Reason = "r" },
		"overlong detail": func(e *DegradationParkEvent) { e.FailureDetail = strings.Repeat("x", 2_049) },
	} {
		event := valid
		mutate(&event)
		if _, err := canonicalDegradationParkEvent(event); !IsCode(err, "INVALID_PARK_EVENT") {

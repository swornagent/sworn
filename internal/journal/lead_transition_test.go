...
func TestAppendEventOnceLinearizesConcurrentLeadRefusal(t *testing.T) {
	store, run, _, _ := journalFixture(t)
	ctx := context.Background()
	now := time.Unix(1_700_000_075, 0).UTC()
	body := []byte(`{"schema_version":"sworn.lead-plan-refusal-event/v1"}`)
	identity := Command{RunID: run.ID, ReplayKey: "lead-refusal/" + strings.Repeat("a", 64), Kind: "lead_refusal", Payload: body, CreatedAt: now}
	const callers = 24
	...
	for index := 0; index < callers; index++ {
		...
		go func() {
			defer wait.Done()
			errors <- store.AppendEventOnce(ctx, identity, "lead_plan_refused", body, now)
		}()
	}
	wait.Wait()
	close(errors)
...
func TestDelegatedBootstrapIsAtomicBeforeAuthorityAdmission(t *testing.T) {
	...
	authority := Command{RunID: run.ID, ReplayKey: "same", Kind: "lead_delegation", Payload: []byte("authority"), CreatedAt: now}
	effect := Effect{RunID: run.ID, ID: "authority", ReplayKey: "same", Kind: "lead.delegation", BeforeDigest: digest(authority.Payload), ExpectedDigest: digest([]byte("result")), UpdatedAt: now}
...
func TestCompleteWithChildIsAtomicAndEventOffsetFencesAuthority(t *testing.T) {
	...
	completion := Completion{RunID: run.ID, EffectID: effect.ID, Token: claim.Token, State: Succeeded, Result: []byte("decision"), EventKind: "lead_plan_decided", EventBody: []byte("safe"), At: now, ExpectedEventOffset: &stale}
...
func TestLeadCompletionLinearizesAgainstRevocationAndReplacement(t *testing.T) {
	for _, mutation := range []string{"revocation", "replacement"} {
		for _, winner := range []string{"lead", mutation} {
			t.Run(mutation+"_"+winner+"_wins", func(t *testing.T) {
				store, run, _, leadEffect := journalFixture(t)
				ctx := context.Background()
				now := time.Unix(1_700_000_200, 0).UTC()
				mutationCommand := Command{RunID: run.ID, ReplayKey: mutation, Kind: "lead_delegation", Payload: []byte(mutation), CreatedAt: now}
				mutationEffect := Effect{RunID: run.ID, ID: mutation, ReplayKey: mutation, Kind: "lead.delegation", BeforeDigest: digest([]byte(mutation)), ExpectedDigest: digest([]byte("changed")), UpdatedAt: now}
				if err := store.RecordCommandEffect(ctx, mutationCommand, mutationEffect); err != nil {
					t.Fatal(err)
				}
				leadClaim, err := store.Claim(ctx, run.ID, leadEffect.ID, now, time.Minute)
				if err != nil {
					t.Fatal(err)
				}
				...
				leadCompletion := Completion{RunID: run.ID, EffectID: leadEffect.ID, Token: leadClaim.Token, State: Succeeded, Result: []byte("proceed"), EventKind: "lead_plan_decided", EventBody: []byte("safe"), At: now, ExpectedEventOffset: &offset}
				mutationCompletion := Completion{RunID: run.ID, EffectID: mutationEffect.ID, Token: mutationClaim.Token, State: Succeeded, Result: []byte("changed"), EventKind: "lead_delegation_" + mutation, EventBody: []byte(mutation), At: now, ExpectedEventOffset: &offset}
				if winner == "lead" {
					if err := store.CompleteWithChild(ctx, leadCompletion, &childCommand, &childEffect); err != nil {
						t.Fatal(err)
					}
					...
					if err := store.CompleteWithChild(ctx, leadCompletion, &childCommand, &childEffect); !IsCode(err, "STALE_COMPLETION") {
						t.Fatalf("lead loser = %v", err)
					}
...
func TestReviseSupersessionEventCrashCutReplaysOneContinuation(t *testing.T) {
	...
	offset := int64(0)
	completion := Completion{RunID: run.ID, EffectID: decision.ID, Token: claim.Token, State: Succeeded, Result: []byte("revise"), EventKind: "lead_plan_decided", EventBody: []byte("safe revise"), At: now, ExpectedEventOffset: &offset}
	if err := store.CompleteWithChild(ctx, completion, &childCommand, &childEffect); err != nil {
		t.Fatal(err)
	}

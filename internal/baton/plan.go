...
// Criterion is one immutable acceptance statement copied from the plan.
type Criterion struct {
	ID   string
	Text string
}

// WorkScope is one copied product include/exclude set.
type WorkScope struct {
	Include []string
	Exclude []string
}

// Work is one copied plan work item.
type Work struct {
	ID          string
	Outcome     string
	Scope       WorkScope
...
// Track is one copied plan track.
type Track struct {
	ID            string
	Ref           string
	DependsOn     []string
...
// Metadata is a detached copy of validated plan metadata.
...
// Plan is an opaque immutable admission. Only ParsePlan can mint one.
type Plan struct {
	admission *planAdmission
}

// ParsePlan admits exact plan bytes whose opening fence begins at byte zero.
...
// DigestBytes returns Baton's lowercase SHA-256 identifier.
...
// FindTrack returns a detached plan track.
...
// FindWork returns detached owning track and work values.
...
func WorkStatusPath(plan Plan, workID string) string {
	return plan.Metadata().RecordRoot + "/" + plan.Metadata().Release + "/work/" + workID + "/status.json"
}

func WorkDesignPath(plan Plan, workID string) string {
	return plan.Metadata().RecordRoot + "/" + plan.Metadata().Release + "/work/" + workID + "/design.md"
}

func WorkProofPath(plan Plan, workID string) string {
	return plan.Metadata().RecordRoot + "/" + plan.Metadata().Release + "/work/" + workID + "/proof.md"
}

func AssemblyStatusPath(plan Plan) string {
	return plan.Metadata().RecordRoot + "/" + plan.Metadata().Release + "/assembly/status.json"
}

func AssemblyProofPath(plan Plan) string {
	return plan.Metadata().RecordRoot + "/" + plan.Metadata().Release + "/assembly/proof.md"
}

func ReleasePlanPath(plan Plan) string {
	return plan.Metadata().RecordRoot + "/" + plan.Metadata().Release + "/plan.md"
}

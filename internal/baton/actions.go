...
type actionHeads struct {
	Target, Release CapturedRef
	Tracks          []CapturedRef
	TrackIDs        []string
}

func refExists(value CapturedRef) bool { return value.State == "direct-commit" }
...
func (a *Actions) captureSnapshot() (Snapshot, actionHeads, error) {
	reader, err := NewReader(a.plan, a.repository)
	if err != nil {
		return Snapshot{}, actionHeads{}, err
	}
	snapshot, err := reader.Capture()
	if err != nil {
		return Snapshot{}, actionHeads{}, err
	}
	heads, err := headsFromRefs(a.plan.Metadata(), snapshot.refs)
	return snapshot, heads, err
}
func headsFromRefs(metadata Metadata, refs []CapturedRef) (actionHeads, error) {
	captured := make(map[string]CapturedRef, len(refs))
	for _, value := range refs {
		captured[value.Ref] = value
	}
	trackIDs := make([]string, len(metadata.Tracks))
	tracks := make([]CapturedRef, len(metadata.Tracks))
	for index, track := range metadata.Tracks {
		trackIDs[index] = track.ID
		tracks[index] = captured[track.Ref]
	}
	result := actionHeads{
		Target: captured[metadata.TargetRef], Release: captured[metadata.ReleaseRef], Tracks: tracks,
		TrackIDs: trackIDs,
	}
	if result.Target.Ref == "" || result.Release.Ref == "" {
		return actionHeads{}, recordFail("INVALID_REF_SNAPSHOT", "snapshot lacks target or release")
	}
	return result, nil
}
func (a *Actions) verifyOperations(before actionHeads, except map[string]bool) []RefOperation {
	...
	for _, ref := range all {
		...
		result = append(result, RefOperation{
			Kind: "verify", Ref: ref.Ref, ExpectedHead: ref.Head,
		})
	}
	return result
}
func (a *Actions) prepareRecord(parent, message string, changes []RepositoryChange) (PreparedRecord, error) {
	...
	beforeProduct, err := a.repository.ProductTreeIdentity(parent)
	if err != nil {
		return PreparedRecord{}, err
	}
	afterProduct, err := a.repository.ProductTreeIdentity(prepared.Commit)
	if err != nil {
		return PreparedRecord{}, err
	}
...
func (a *Actions) InstallApprovedPlan(approvalDigest string) (Receipt, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	captured, before, err := a.captureSnapshot()
	if err != nil {
		return Receipt{}, err
	}
	metadata := a.plan.Metadata()
	if !refExists(before.Target) {
	...
	if !refExists(before.Release) {
		...
		for _, track := range before.Tracks {
			operations = append(operations, RefOperation{Kind: "verify", Ref: track.Ref})
		}
		if err := a.repository.AtomicUpdateRefs(captured.refs, operations); err != nil {
			return Receipt{}, err
		}
		...
		if !refExists(after.Release) || after.Release.Head != prepared.Commit {
...
func (a *Actions) ReboundPristinePlan(previousPlan Plan, approvalDigest string) (Receipt, error) {
	...
	metadata := a.plan.Metadata()
	capturedRefs, err := a.repository.CaptureHeadRefs(planRefs(metadata))
	if err != nil {
		return Receipt{}, err
	}
	before, err := headsFromRefs(metadata, capturedRefs)
	if err != nil {
		return Receipt{}, err
	}
	if !refExists(before.Release) {
	...
	if installedPlan.Digest() == a.plan.Digest() {
		...
		current, err := currentReader.capture(capturedRefs)
		if err != nil {
			return Receipt{}, err
		}
		for _, track := range a.plan.Metadata().Tracks {
	...
	snapshot, err := reader.capture(capturedRefs)
	if err != nil {
		return Receipt{}, err
	}
...
type Handoffs struct{ Design, Proof []byte }
...
func (a *Actions) RecordTransition(input RecordTransitionInput) (Receipt, error) {
	...
	if ref := findCaptured(after, selected.Ref); !refExists(ref) || ref.Head != prepared.Commit {

...
func TestCompleteSevenActionLoopAndExactRetries(t *testing.T) {
	for _, format := range []gitx.ObjectFormat{gitx.SHA1, gitx.SHA256} {
		...
		t.Run(string(format), func(t *testing.T) {
			...
			if err != nil || len(ownerRefs) != 1 || ownerRefs[0].Head.IsZero() {
			...
			productTree, err := mechanical.ProductTreeIdentity(candidate.Commit)
			if err != nil {
				t.Fatal(err)
			}
			...
			statusForCandidate := func(prepared gitx.PreparedCommit) Status {
				t.Helper()
				product, err := mechanical.ProductTreeIdentity(prepared.Commit)
				if err != nil {
					t.Fatal(err)
				}
			...
			if err != nil || len(implementedHeads) != 1 || implementedHeads[0].Head.IsZero() {
			...
			finalHeads, err := repository.CaptureHeadRefs([]string{
				"refs/heads/release/v0.3.0", "refs/heads/release-wt/demo-v1", "refs/heads/track/demo-v1/T1",
			})
			if err != nil {
				t.Fatal(err)
			}
			finalByRef := make(map[string]CapturedRef, len(finalHeads))
			for _, value := range finalHeads {
				finalByRef[value.Ref] = value
			}
			targetFinal := finalByRef["refs/heads/release/v0.3.0"]
			releaseFinal := finalByRef["refs/heads/release-wt/demo-v1"]
			if targetFinal.Head == "" || releaseFinal.Head == "" ||
				targetFinal.Head == targetHead || targetFinal.Head == releaseFinal.Head {
				t.Fatalf("target/release terminal identities are not distinct: %#v", finalHeads)
			}
			for _, expected := range golden.FinalRefs {
				observed, ok := finalByRef[expected.Ref]
				if !ok || expected.Head == nil || observed.Head != *expected.Head {
					t.Fatalf("final ref %s differs from oracle: %#v want %#v", expected.Ref, finalHeads, golden.FinalRefs)
				}
			}

...
const (
	...
	MaxStatusBytes     = 262_144
	MaxEvidenceBytes   = 1_048_576
	MaxTracks          = 64
	MaxWorkPerTrack    = 256
	MaxWorkTotal       = 1024
	MaxListItems       = 256
	MaxProofComponents = 64
)

// RecordError retains the stable Baton error code while keeping prose out of
// the contract.
...
func recordFail(code, message string) error {
	return &RecordError{Code: code, Msg: message}
}
...
func decodeStrictValue(decoder *json.Decoder, depth int) (any, error) {
	...
	case json.Number:
		...
		if math.Trunc(number) == number && math.Abs(number) > 9_007_199_254_740_991 {
			return nil, recordFail("UNSAFE_INTEGER", "integer-valued number outside interoperable range: "+raw)
		}
		return value, nil

...
// CandidateValidation binds every Git and product identity checked for one
// work candidate.
...
// ValidateWorkCandidate replays first-parent candidate history, enforces
// record/product separation and scope, and binds proof identities to Git.
func ValidateWorkCandidate(
	...
	productTree, err := repository.ProductTreeIdentity(proof.CandidateCommit, metadata.RecordRoot)
	if err != nil {
		return CandidateValidation{}, err
	}
...
// ValidateWorkRecordTail proves every direct commit after the candidate is a
// single-parent record-only handoff for this work and contains no hidden
// product change.

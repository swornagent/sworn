...
// Profile selects the protected-evidence strength required by an action.
type Profile string
...
// EvidenceProvenance is closed typed provenance returned by the trusted
// resolver. Fields unused by a kind must retain their zero value.
type EvidenceProvenance struct {
	...
	Decision           string
	PlanDigest         string
	AuthorizerIsolated bool
	DeliveryWritable   bool

	Role             string
	FreshContext     bool
	ReadOnly         bool
	EngineControlled bool
	Invocation       string
	ProofDigest      string
	CandidateCommit  string
	ProductTree      string
...
// InertnessRequest binds product exclusion to one exact repository, root and
// candidate commit.
type InertnessRequest struct {
	Repository string
	RecordRoot string
	Commit     string
}

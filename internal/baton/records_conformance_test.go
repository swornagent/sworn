...
func TestBatonV015ExactParity(t *testing.T) {
	...
	assertExactArchiveInventory(t, sourceRoot, archive)
	repo := repoRoot(t)
...
func TestBatonV015CodexAndClaudeMirrorParity(t *testing.T) {
	archiveBytes := adopt.BatonInstallerArchive()
	// The exact installer scripts come from the authenticated temporary clone;
	// installer_archive_test independently exercises the safely extracted
	// embedded archive against the same native generator.
	bundle := cloneVerifiedBatonV015Bundle(t)
	...
	assertContainedDisjointInstallerProofRoots(t, base, home, roots.AgentsHome, roots.CodexHome, roots.ClaudeHome, roots.RecoveryRoot)
	env := installerProofEnvironment(base, home, roots)
	runPinnedInstallerScript(t, bundle, "install-codex.sh", env)
	runPinnedInstallerScript(t, bundle, "install-claude.sh", env)
...
func TestBatonV015ParityFailsClosedOnEveryLayer(t *testing.T) {
	...
	t.Run("upstream VERSION object", func(t *testing.T) {
		source, repository := newPinnedParityFixture(t)
		if err := os.WriteFile(filepath.Join(source, "VERSION"), []byte("v0.15.1"), 0o644); err != nil {
			t.Fatal(err)
		}
		if _, err := baton.Diff(baton.DiffOpts{SourceDir: source, RepoRoot: repository}); err == nil ||
			!strings.Contains(err.Error(), "pinned Baton checkout is dirty") {
...
func cloneVerifiedBatonV015Bundle(t *testing.T) string {
	...
	bundlePath := filepath.Join(repoRoot(t), "internal", "baton", "testdata", "fixture", "baton-v0.15.1.bundle")
	...
	if got := hex.EncodeToString(digest[:]); got != bundleSHA256 {
		t.Fatalf("bundle SHA-256 = %s, want %s", got, bundleSHA256)
	}
	if got := gitBlobOIDForTest(bundle); got != bundleBlob {
	...
	if len(bundle) < len(header) || string(bundle[:len(header)]) != header {
		t.Fatal("bundle header differs from the authenticated v0.15.1 source")
	}
	...
	if err != nil || !strings.Contains(string(verifyOutput), "complete history") {
		t.Fatalf("verify committed bundle: %v\n%s", err, verifyOutput)
	}
	...
	if output, err := exec.Command("git", "clone", "--no-checkout", bundlePath, clone).CombinedOutput(); err != nil {
		t.Fatalf("clone authenticated bundle: %v\n%s", err, output)
	}
	runVerifiedBundleGit(t, clone, "fsck", "--full", "--no-reflogs")
	runVerifiedBundleGit(t, clone, "checkout", "--detach", "v0.15.1^{commit}")
	if got := verifiedBundleGitOutput(t, clone, "rev-parse", "v0.15.1^{tag}"); got != tagObject {
		t.Fatalf("annotated tag object = %s, want %s", got, tagObject)
	}
	if got := verifiedBundleGitOutput(t, clone, "rev-parse", "v0.15.1^{commit}"); got != commit {
		t.Fatalf("peeled tag commit = %s, want %s", got, commit)
	}
	if got := verifiedBundleGitOutput(t, clone, "rev-parse", "HEAD"); got != commit {
		t.Fatalf("checked-out HEAD = %s, want %s", got, commit)
	}
	if got := verifiedBundleGitOutput(t, clone, "rev-parse", commit+":VERSION"); got != versionBlob {
		t.Fatalf("upstream VERSION blob = %s, want %s", got, versionBlob)
	}
	version, err := os.ReadFile(filepath.Join(clone, "VERSION"))
	if err != nil || !bytes.Equal(version, []byte("v0.15.1\n")) {
		t.Fatalf("upstream VERSION bytes = %q err=%v", version, err)
	}
	if status := verifiedBundleGitOutput(t, clone, "status", "--porcelain=v1", "--untracked-files=all"); status != "" {
		t.Fatalf("fresh authenticated clone is dirty: %q", status)
	}
	return clone
}

func runVerifiedBundleGit(t *testing.T, clone string, args ...string) {
	t.Helper()
...
func verifiedBundleGitOutput(t *testing.T, clone string, args ...string) string {
	t.Helper()
...
func assertExactArchiveInventory(t *testing.T, clone string, archive *baton.InstallerArchive) {
	t.Helper()
	assertArchiveMatchesVerifiedClone(t, clone, archive)
...
// assertArchiveMatchesVerifiedClone compares every archive path, mode, and
// blob directly to the authenticated temporary clone. This is deliberately
// stronger than checking the pinned aggregate digest or entry count alone.
func assertArchiveMatchesVerifiedClone(t *testing.T, clone string, archive *baton.InstallerArchive) {
	t.Helper()
	args := []string{"-C", clone, "ls-tree", "-r", "-z", baton.PinnedBatonCommit, "--",
		"install-codex.sh", "install-claude.sh", "baton", "commands", "schemas"}
	output, err := exec.Command("git", args...).Output()
	if err != nil {
		t.Fatalf("list pinned installer source tree: %v", err)
	}
	type sourceEntry struct {
		mode os.FileMode
		blob string
	}
	files := make(map[string]sourceEntry)
	...
	for _, raw := range bytes.Split(output, []byte{0}) {
		...
		if len(parts) != 2 {
			t.Fatalf("malformed git ls-tree record %q", raw)
		}
		fields := strings.Fields(string(parts[0]))
		if len(fields) != 3 || fields[1] != "blob" {
			t.Fatalf("unexpected git ls-tree metadata %q", parts[0])
		}
		...
		case "100755":
			mode = 0o775
		default:
			t.Fatalf("unexpected pinned Git mode %q for %q", fields[0], parts[1])
		}
		name := string(parts[1])
		files[name] = sourceEntry{mode: mode, blob: fields[2]}
	...
	for _, entry := range archive.Entries {
		if entry.IsDir {
			if _, ok := directories[entry.Path]; !ok {
				t.Errorf("archive directory %q is absent from the verified clone inventory", entry.Path)
			} else {
				delete(directories, entry.Path)
			}
			if entry.Mode != 0o775 {
				t.Errorf("archive directory %q mode = %04o, want 0775", entry.Path, entry.Mode)
			}
			continue
		}
		want, ok := files[entry.Path]
		if !ok {
			t.Errorf("archive file %q is absent from the verified clone inventory", entry.Path)
			continue
		}
		delete(files, entry.Path)
		if entry.Mode != want.mode {
			t.Errorf("archive file %q mode = %04o, want %04o from verified clone", entry.Path, entry.Mode, want.mode)
		}
		if entry.BlobOID != want.blob {
			t.Errorf("archive file %q blob = %s, want %s from verified clone", entry.Path, entry.BlobOID, want.blob)
		}
	}
	for name := range files {
		t.Errorf("verified clone file %q is missing from archive inventory", name)
	}
	for name := range directories {
		t.Errorf("verified clone directory %q is missing from archive inventory", name)
	}
}
...
func installerProofEnvironment(base, home string, roots baton.InstallRoots) []string {
...
func assertContainedDisjointInstallerProofRoots(t *testing.T, base string, roots ...string) {
	t.Helper()
	...
	for i, root := range roots {
		...
		if !installerPathContained(base, root) {
			t.Fatalf("installer proof root escapes test root: %s", root)
		}
		if installerPathsOverlap(realHome, root) {
			t.Fatalf("installer proof selected real home: %s", root)
		}
		for _, name := range []string{".agents", ".codex", ".claude"} {
			if installerPathsOverlap(filepath.Join(realHome, name), root) {
				t.Fatalf("installer proof selected real %s home: %s", name, root)
			}
		}
		for _, other := range roots[:i] {
			if installerPathsOverlap(root, other) {
				t.Fatalf("installer proof roots overlap: %s and %s", root, other)
			}
		}
	}
}

func installerPathContained(base, candidate string) bool {
	rel, err := filepath.Rel(base, candidate)
	return err == nil && rel != ".." && !strings.HasPrefix(rel, ".."+string(filepath.Separator))
}

func installerPathsOverlap(left, right string) bool {
	left, _ = filepath.Abs(left)
	right, _ = filepath.Abs(right)
	return installerPathContained(left, right) || installerPathContained(right, left)
}

...
// MaterializeTrack atomically advances release-wt and creates one owner at the
// same collective record-only marker.
...
// ComposeTrack deterministically composes one passed owner, then advances only
// release-wt with a direct collective transfer record.
...
// PrepareAssembly records the exact composed release candidate and ordered
// component heads, then hands it to a fresh Verifier.
func (a *Actions) PrepareAssembly(input PrepareAssemblyInput) (Receipt, error) {
	...
	product, err := a.repository.ProductTreeIdentity(candidate, metadata.RecordRoot)
	if err != nil {
		return Receipt{}, err
	}
	components := make([]ProofComponent, len(metadata.Tracks))
	for index, track := range metadata.Tracks {
		if !before.Tracks[index].Exists {
...
// IntegrateRelease atomically advances the target and release terminal status
// only from an exact fresh assembly PASS.
func (a *Actions) IntegrateRelease() (Receipt, error) {
	...
	if projection(previous.View()) == "merge/complete/none" && previous.View().Outcome == "merged" {
		...
		if !before.Target.Exists || before.Target.Head != merge.ResultCommit {
...
func (a *Actions) validateAssembly(status Status, authorityHead string, heads actionHeads, snapshot Snapshot) error {
	...
	product, err := a.repository.ProductTreeIdentity(view.Proof.CandidateCommit, metadata.RecordRoot)
	if err != nil {
		return err
	}
	...
	for index, track := range metadata.Tracks {
		if !heads.Tracks[index].Exists || !reflect.DeepEqual(view.Proof.Components[index], ProofComponent{TrackID: track.ID, Head: heads.Tracks[index].Head}) {

...
// StatusView is a detached copy of one strictly admitted durable status.
// Optional fields are pointers so absence remains distinguishable from zero.
...
// Status is an opaque immutable strict record. Only ParseStatus can mint one.
type Status struct {
...
// ParseStatus strictly parses and semantically admits exact status bytes.

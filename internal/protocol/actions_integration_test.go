...
func loadActionGolden(t *testing.T) actionGoldenFile {
	t.Helper()
	body, err := os.ReadFile(filepath.Join("..", "..", "tools", "protocolgolden", "testdata", "corpus", "actions.json"))
	if err != nil {
		t.Fatal(err)
	}
...
func loadStateGolden(t *testing.T) stateGoldenFile {
	t.Helper()
	body, err := os.ReadFile(filepath.Join("..", "..", "tools", "protocolgolden", "testdata", "corpus", "state.json"))
	if err != nil {
		t.Fatal(err)
	}
...
func loadGitGolden(t *testing.T) gitGoldenFile {
	t.Helper()
	body, err := os.ReadFile(filepath.Join("..", "..", "tools", "protocolgolden", "testdata", "corpus", "git.json"))
	if err != nil {
		t.Fatal(err)
	}
...
func actionEnvironment(repo string, extra ...string) []string {
	...
	base := []string{
		...
		"GIT_AUTHOR_NAME=Golden Protocol Engine",
		"GIT_AUTHOR_EMAIL=engine@example.test",
		"GIT_AUTHOR_DATE=1000000000 +0000",
		"GIT_COMMITTER_NAME=Golden Protocol Engine",
...
func createActionHarness(t *testing.T) (string, *gitx.Repository, *Actions) {
	...
	actions, err := NewActions(UseGitRepository(repository), inertActionResolver, gitx.Identity{Name: "Golden Protocol Engine", Email: "engine@example.test"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestHistoricalMixedCommitIdentitiesReconstructExactly(t *testing.T) {
	...
	if !ok || slice.Stage != "design" || slice.NextRole != "lead" {
...
func TestUnverifiedCandidateHeadRefreshReturnsToImplementerWithoutVerdict(t *testing.T) {
	...
	appendActionReceipt(t, actions, AppendReceiptInput{
		Release: release, Slice: "S1", Role: "lead", Result: "proceed",
...
func TestActionsMatchExactReferenceForSHA1AndSHA256(t *testing.T) {
	...
	for flowIndex, expectedFlow := range golden.Flows {
		...
		t.Run(expectedFlow.ObjectFormat, func(t *testing.T) {
			...
			actions, err := NewActions(UseGitRepository(gitRepository), inertActionResolver, gitx.Identity{Name: "Golden Protocol Engine", Email: "engine@example.test"})
			if err != nil {
				t.Fatal(err)
			}
...
func TestCandidateReceiptRejectsReservedRecordRootChangeFromPreparedBase(t *testing.T) {
	...
	candidate := commitActionProduct(
		...
		parent,
		ref,
		".protocol/releases/foreign/plan.md",
		"reserved\n",
		1000000500,
	)
...
func TestAttemptTransitionsAndDirectSingleTrackMerge(t *testing.T) {
	...
	appendActionReceipt(t, actions, AppendReceiptInput{
		Release: release, Slice: "S1", Role: "lead", Result: "revise",
...
func TestReviewedPinsRetainCrossPlanStagesWhenProductsAreUnchanged(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if test.decision != "" {
				appendActionReceipt(t, actions, AppendReceiptInput{
					Release: release, Slice: "S2", Role: "lead", Result: test.decision,
			...
			wantRole, wantStage, wantOutcome, wantAttempt := "lead", "design", "none", int64(1)
...
func TestChangedReviewedProductReroutesEveryLaterStageToFreshDesign(
	...
	for _, stage := range []string{"designed", "candidate", "failed", "passed"} {
		...
		t.Run(stage, func(t *testing.T) {
			...
			if stage != "designed" {
				appendActionReceipt(t, actions, AppendReceiptInput{
					Release: release, Slice: "S2", Role: "lead",
...
func TestConsumedDriftCannotOverridePlannerBlocker(t *testing.T) {
	for _, blocker := range []string{"lead", "verifier"} {
		blocker := blocker
...
func TestPlannerBlockedOutcomesResetAcrossRevision(t *testing.T) {
	for _, result := range []string{"escalate", "blocked"} {
		...
		t.Run(result, func(t *testing.T) {
			...
			appendActionReceipt(t, actions, AppendReceiptInput{
				Release: release, Slice: "S1", Role: "lead",
				Result: func() string {
					if result == "escalate" {
						return result
					}

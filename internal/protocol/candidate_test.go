...
func runProtocolTestGit(t *testing.T, repository string, args ...string) string {
	t.Helper()
...
func TestValidateSliceCandidateScopeRefusalsCarryBoundedPaths(t *testing.T) {
	tempDir := t.TempDir()
	runProtocolTestGit(t, tempDir, "init", "--initial-branch=main")
	runProtocolTestGit(t, tempDir, "config", "user.name", "Fixture")
	runProtocolTestGit(t, tempDir, "config", "user.email", "fixture@example.invalid")
	...
	runProtocolTestGit(t, tempDir, "add", "base.txt")
	runProtocolTestGit(t, tempDir, "commit", "-m", "initial")
	baseOID := runProtocolTestGit(t, tempDir, "rev-parse", "HEAD")
	...
	t.Run("out-of-scope files bounded to 20", func(t *testing.T) {
		runProtocolTestGit(t, tempDir, "checkout", "-B", "out-of-scope-branch", baseOID)
		for i := 0; i < 25; i++ {
			...
			runProtocolTestGit(t, tempDir, "add", filepath.ToSlash(filepath.Join("pkg", "disallowed", fmt.Sprintf("file_%02d.txt", i))))
		}
		runProtocolTestGit(t, tempDir, "commit", "-m", "25 out of scope changes")
		candidateOID := runProtocolTestGit(t, tempDir, "rev-parse", "HEAD")
	...
	t.Run("reserved-root files bounded to 20", func(t *testing.T) {
		runProtocolTestGit(t, tempDir, "checkout", "-B", "reserved-root-branch", baseOID)
		for i := 0; i < 25; i++ {
			...
			runProtocolTestGit(t, tempDir, "add", filepath.ToSlash(filepath.Join(gitx.DefaultRecordsRoot, "rel-1", fmt.Sprintf("escape_%02d.md", i))))
		}
		runProtocolTestGit(t, tempDir, "commit", "-m", "25 reserved root changes")
		candidateOID := runProtocolTestGit(t, tempDir, "rev-parse", "HEAD")
	...
	t.Run("in-scope change passes", func(t *testing.T) {
		runProtocolTestGit(t, tempDir, "checkout", "-B", "valid-branch", baseOID)
		...
		runProtocolTestGit(t, tempDir, "add", "pkg/allowed/feature.txt")
		runProtocolTestGit(t, tempDir, "commit", "-m", "valid change")
		candidateOID := runProtocolTestGit(t, tempDir, "rev-parse", "HEAD")

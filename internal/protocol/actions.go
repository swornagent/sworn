...
// protocol.plan/v2 plans carry their full slice bodies inline and ignore
// ContractTree.
//
...
func actionResult(action string, changed bool) ActionResult {
	return ActionResult{Kind: "protocol.action-result/v2", Action: action, Changed: changed}
}
...
// protocol.plan/v2 plans carry their slices inline and publish only the plan.
...
// documents root is what a person reads. Legacy protocol.plan/v2 plans carry
...
func (a *Actions) appendReceipt(
	...
	if sliceID != "" {
		...
		case role == "lead" && (resultName == "proceed" || resultName == "revise" || resultName == "escalate"):
			if slice.NextRole != "lead" {
				return ActionResult{}, recordFail("ROLE_NOT_ELIGIBLE", sliceID+" does not currently need Lead review")
			}

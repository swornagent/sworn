...
var declaredS5Checks = []string{
	...
	"GOFLAGS=-buildvcs=false go test -count=1 -race ./internal/driver ./internal/protocol",
	"GOFLAGS=-buildvcs=false go vet ./...",
	"gofmt -l ./cmd ./internal ./tools",
}

// TestCheckCommandCoversAgainstRealDeclaredChecks pins finding 1 of the S5-A3
// lead review: the matching rule is satisfiable by a real command a
	...
	tests := []struct {
		...
		},
		{
			...
			recorded: "GOFLAGS=-buildvcs=false /usr/local/go/bin/go test -count=1 -race " +
				"./internal/driver ./internal/protocol > /tmp/s5-check2.log 2>&1",
			want: true,
		},
		{

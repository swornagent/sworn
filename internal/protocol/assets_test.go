...
func TestLoadAdmitsSwornOwnedRoleAssets(t *testing.T) {
	...
	if string(version) != LegacyProtocolVersion+"\n" {
...
// tagged, installed, or certified external Protocol release. The absence of
...
func TestManifestIdentityBindsLegacyProvenanceNotAdmission(t *testing.T) {
	...
	if manifest.Commit != LegacyProtocolCommit {
		t.Fatal("asset manifest does not preserve the legacy Protocol source commit as provenance")
	}
...
func TestReadAssetReturnsIndependentBytes(t *testing.T) {
	...
	if string(second) != LegacyProtocolVersion+"\n" {
...
func TestCompiledInventoryIsClosed(t *testing.T) {
	...
	for _, name := range got {
		if bytes.Contains([]byte(name), []byte(".protocol/releases")) ||
...
// separately installed, tagged, or certified external Protocol release; the

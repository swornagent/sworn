...
func TestHistoricalScopeRegressionCorpus(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// Touchpoints: internal/runtime, internal/protocol, internal/journal
			// Missing: internal/driver (whose tests import internal/protocol)
			slice: Slice{
				ID: "S1",
				Scope: Scope{
					Include: []string{"internal/runtime", "internal/protocol", "internal/journal"},
				},
			},
			wantMissing:  []string{"internal/driver"},
			wantReasonIn: []string{"test imports internal/protocol"},
		},
		{
			...
			// Missing: internal/protocol and internal/skill (which import internal/gitx)
			slice: Slice{
				ID: "S1",
				Scope: Scope{
			...
			wantMissing:  []string{"internal/protocol", "internal/skill"},
		...
		},
		{
			...
			// Touchpoints: internal/protocol, internal/gitx, internal/driver, internal/runtime
			// Missing: cmd/sworn, internal/skill, tools/protocolgolden
			slice: Slice{
				ID: "S2",
				Scope: Scope{
					Include: []string{"internal/protocol", "internal/gitx", "internal/driver", "internal/runtime"},
				},
			},
			wantMissing:  []string{"cmd/sworn", "internal/skill", "tools/protocolgolden"},
			wantReasonIn: []string{"test imports internal/protocol"},
		},
		{
			name: "Case 4: Golden-corpus gap (2026-08-12-configurable-paths rev3 S2 omitting tools)",
			// Scope touching internal/protocol, internal/gitx, internal/driver, internal/runtime, cmd/sworn, internal/skill but omitting tools
			// Missing: tools/protocolgolden (imports internal/protocol & fixture pins it)
			slice: Slice{
				ID: "S2",
				Scope: Scope{
					Include: []string{"internal/protocol", "internal/gitx", "internal/driver", "internal/runtime", "cmd/sworn", "internal/skill"},
				},
			},
			wantMissing:  []string{"tools/protocolgolden"},
			wantReasonIn: []string{"fixture tooling pins internal/protocol", "test imports internal/protocol"},
		},
	}
...
func TestScopeWaiversPermitAdmissionAndPreserveBytes(t *testing.T) {
	...
	waivedSlice := Slice{
		...
		Scope: Scope{
			...
			Waivers: []ScopeWaiver{
				...
				{Package: "tools/protocolgolden", Reason: "Golden corpus decoupled"},
			},
		},
	}
	...
	// Test hierarchical waiver matching (e.g. waiving "tools" covers "tools/protocolgolden")
	if !isWaived("tools/protocolgolden", []ScopeWaiver{{Package: "tools", Reason: "All tools waived"}}) {
		t.Fatal("expected 'tools' waiver to match 'tools/protocolgolden'")
	}
}
...
func TestCanonicalDigestInvarianceWithoutWaivers(t *testing.T) {
	...
	contractValue := map[string]any{
		"outcome":     "Deliver S1.",
		"scope":       map[string]any{"include": []any{"internal/protocol"}, "exclude": []any{}},
	...
	contractValueWithEmptyWaivers := map[string]any{
		"outcome":     "Deliver S1.",
		"scope":       map[string]any{"include": []any{"internal/protocol"}, "exclude": []any{}, "waivers": []any{}},
		"acceptance":  []any{map[string]any{"id": "A1", "text": "Text"}},
		"checks":      []any{"check 1"},
		"constraints": []any{"constraint 1"},
		"depends_on":  []any{},
	...
	contractValueWithWaiver := map[string]any{
		...
		"scope": map[string]any{
			"include": []any{"internal/protocol"},
			"exclude": []any{},
			"waivers": []any{
				map[string]any{
					"package": "tools/protocolgolden",
					"reason":  "Golden tools waived for standalone protocol test",
				},
			},
		},
		"acceptance":  []any{map[string]any{"id": "A1", "text": "Text"}},
	...
	if len(waivedSlice.Scope.Waivers) != 1 || waivedSlice.Scope.Waivers[0].Package != "tools/protocolgolden" ||
		waivedSlice.Scope.Waivers[0].Reason != "Golden tools waived for standalone protocol test" {
	...
	}{
		{
			...
			raw: `{
				...
				"scope": {
					...
					"exclude": [],
					"waivers": [{"package": "tools/protocolgolden"}]
				},
		...
		},
		{
			...
			raw: `{
				...
				"scope": {
					"include": ["internal/protocol"],
					"exclude": [],
					"waivers": [{"package": "tools/protocolgolden", "reason": ""}]
				},
				"acceptance": [{"id": "A1", "text": "T"}],
				"checks": ["c"], "constraints": ["c"], "depends_on": [], "consumes": []
			}`,
		...
		},
		{
			...
			raw: `{
				...
				"scope": {
					...
					"exclude": [],
					"waivers": [
						{"package": "tools/protocolgolden", "reason": "reason 1"},
						{"package": "tools/protocolgolden", "reason": "reason 2"}
					]
				},
				"acceptance": [{"id": "A1", "text": "T"}],
				"checks": ["c"], "constraints": ["c"], "depends_on": [], "consumes": []
...
func TestRecordPlanRevisionScopeLintRefusesUnderDerivedPlan(t *testing.T) {
	...
	withModule := prepareActionContractTree(t, repoPath, base, map[string][]byte{
		...
		"internal/protocol/protocol.go": protocolFile,
		"internal/driver/main.go":       driverFile,
	})
	actionGit(t, repoPath, nil, nil, "update-ref", "refs/heads/main", withModule, base)

	// Under-derived plan: touching internal/protocol, omitting internal/driver
	...
	planBytes := manifestActionPlanBytes(t, release, contractPath, "internal/protocol", digest, []any{})
...
func TestRecordPlanRevisionWithWaiversSucceeds(t *testing.T) {
	...
	entry := manifestSliceEntry("S1", contractPath, "internal/protocol", digest)
	entry["waivers"] = []any{
		map[string]any{
			"package": "internal/driver",
			"reason":  "Driver changes deferred to subsequent slice",

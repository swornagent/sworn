...
func TestVerifyAssetDirectsVerifierToReadHostBoundaryEvidence(t *testing.T) {
	...
	body, err := pkg.ReadAsset("operations/protocol-verify.md")
	if err != nil {
		t.Fatal(err)
	}
	text := string(body)
	for _, required := range []string{
		...
		if !strings.Contains(text, required) {
			t.Fatalf("protocol-verify.md is missing %q", required)
		}
	}
	if !strings.Contains(text, "host-evidence.json") {
		t.Fatalf("protocol-verify.md does not name the projected host evidence input")
	}
	...
	for _, operation := range release.Operations {
		if operation.Name != "protocol-verify" {
			continue
		}
		if digest(body) != operation.SHA256 {
			t.Fatalf("protocol-verify asset digest %s != operation pin %s", digest(body), operation.SHA256)
		}
		for _, entry := range manifest.Assets {
			if entry.Path == "operations/protocol-verify.md" &&
				(entry.SHA256 != operation.SHA256 ||
					int64(len(body)) != entry.Size) {
				t.Fatalf("protocol-verify manifest pin does not match shipped bytes")
			}
		}
	}
}

...
func loadReceiptGolden(t *testing.T) receiptGoldenCorpus {
	t.Helper()
	body, err := os.ReadFile(filepath.Join("..", "..", "tools", "protocolgolden", "testdata", "corpus", "receipts.json"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanAdmissionMatchesExactProtocolGolden(t *testing.T) {
	t.Parallel()
...
func TestStrictJSONMatchesExactProtocolGolden(t *testing.T) {
	t.Parallel()
	golden := loadReceiptGolden(t)

...
// no methods by design (TestExternalActionSurfaceIsExactlyTheFourProtocolFacades
...
func (r *repository) history(head string) ([]historyRow, error) {
	...
	for index, row := range rows {
		engineCommit := bytes.HasPrefix(row.Message, []byte("protocol(")) ||
			bytes.HasPrefix(row.Message, []byte("Protocol exact ")) ||
			bytes.HasPrefix(row.Message, []byte("Protocol engine-owned ")) ||

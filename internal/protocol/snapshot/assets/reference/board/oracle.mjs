...
import {
  ProtocolStateError,
  isProtocolStateError,
  readProtocolState,
} from '../records/state.mjs';

export const BOARD_VERSION = 'protocol.board/v1';
export const GRAPH_VERSION = 'protocol.graph/v1';
...
const OPERATION_BY_ROLE = Object.freeze({
  planner: 'protocol-plan',
  implementer: 'protocol-implement',
  lead: 'protocol-design-review',
  verifier: 'protocol-verify',
...
function diagnostic(error, repository, context = {}) {
  const code = isProtocolStateError(error) || error instanceof GitRecordError
    ? error.code
...
export function discoverReleaseHeads(repo) {
  return parseReleaseListing(unsafeRunGit(repo, [
    ...
    encoding: null,
    label: 'discover Protocol release refs',
  }));
}
...
export function createBoardOracle({ readState = readProtocolState } = {}) {
      ...
      const releases = discovered.map((entry) => {
        ...
        for (let attempt = 0; attempt < 2; attempt += 1) {
          try {
            ...
            if (
              error instanceof ProtocolStateError
        ...
        return invalidRelease(
          current,
          new ProtocolStateError('REF_SNAPSHOT_UNSTABLE', `release ${current.release} kept moving`),
          root,
        );
      });

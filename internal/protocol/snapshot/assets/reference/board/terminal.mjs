...
const BOARD_VERSION = 'protocol.board/v1';
...
export function renderTerminal(board, { color = 'auto', isTTY = false } = {}) {
  ...
  lines.push(`Protocol board \u2014 ${paint(useColor, stateColor(validity), tokenLabel(validity))}`);
  ...
  if (board.releases.length === 0) {
    lines.push(paint(useColor, 'dim', 'No active Protocol release was found.'));
    lines.push('Next: ask an agent to start with protocol-plan.');
  }
...
function main(argv) {
  ...
  try {
    ...
    process.stderr.write(
      `Protocol could not read this board. ${sanitizeTerminalText(error?.message ?? 'invalid board')}\n`,
    );

...
<body>
  ...
  <footer class="footer">
    <p>Read-only. Built from saved Protocol records.</p>
  </footer>
</body>
</html>
`;
...
const GRAPH_VERSION = 'protocol.graph/v1';
...
function relayMap(release) {
  ...
  if (!graph.valid) {
    wrapper.append(element(
      'p',
      'graph-unavailable',
      'Protocol could not draw this release safely. See the issue above.'
    ));
    return wrapper;
  }
  ...
  start.append(element('span', 'protocol-mark'));
...
function render(board) {
  ...
  copy.append(element('p', 'eyebrow', 'Protocol board'));
  copy.append(element('h1', 'intro-title', board.repository || 'No active release'));
  copy.append(element(
    'p',
    'intro-copy',
    board.valid
      ? 'Protocol is how the work is handed off. One approved plan starts the relay; each checked piece carries the work forward.'
  ...
  if (!Array.isArray(board.releases) || board.releases.length === 0) {
    ...
    empty.append(element('p', 'empty-copy', 'Ask an agent to start with protocol-plan. An approved release will appear here.'));
...
async function refresh() {
  try {
    ...
    if (!board || board.schema_version !== 'protocol.board/v1') {
...
.brand-mark {
  ...
  box-shadow: inset -11px 0 0 var(--protocol-dark);
...
.protocol-mark {
  ...
  border-radius: 999px;
  background: var(--protocol);
  box-shadow: inset -0.9rem 0 0 var(--protocol-dark);
...
.final-exchange {
  border-top: 3px solid var(--protocol);
  border-bottom: 1px solid var(--ink);
  box-shadow: 4px 4px 0 var(--timing-shadow);
}
...
:focus-visible {
  outline: 3px solid var(--protocol);
  outline-offset: 3px;
}

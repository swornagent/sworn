...
const REF_HELPER_MARKER = '--protocol-exact-ref-helper-v1';
 ...
 * Identity is commit metadata only; Protocol never treats it as role or authority.
 */
...
function executeGit(repo, args, options = {}, internal = {}) {
  const hooksDirectory = mkdtempSync(path.join(tmpdir(), 'protocol-git-hooks-'));
  try {
 ...
 * createProtocolActions; this exists for the engine implementation and
...
export function unsafeAtomicUpdateRefs(repo, operations) {
  ...
  const hooksDirectory = realpathSync(
    mkdtempSync(path.join(tmpdir(), 'protocol-ref-hooks-')),
  );
  ...
  if (observed && exactRefVectorMatches(observed, prepared.desiredState)) {
    ...
    result = new GitRecordError(
      ...
      'exact Protocol ref transaction lost without partial advancement',
      helperOutcome,
    );
  } else {
    result = new GitRecordError(
      'ATOMIC_REF_UPDATE_FAILED',
      'exact Protocol ref transaction has an ambiguous outcome; '
...
function withEngineGitContext(repo, operation) {
  const temporary = mkdtempSync(path.join(tmpdir(), 'protocol-git-context-'));
...
function normalizedCandidateForRecordRoot(
  ...
  return runEngineGit(
    ...
    {
      input: `Protocol engine-owned record normalization for ${candidate}\n`,
...
function exactBaseMergeSide(context, source, productBase, side, identity) {
  ...
  return runEngineGit(
    ...
    {
      input: `Protocol exact-base ${side} for ${source}\n`,
      env: gitIdentityEnvironment(identity, date),
 ...
 *
 * Protocol receipts live in commit messages, so their commit must reuse the
...
function deterministicCompositionCommit(
  ...
  return runEngineGit(
    ...
    {
      input: `Protocol exact composition of ${candidate} into ${targetRef}\n`,
      env: gitIdentityEnvironment(identity, date),
...
export function unsafePrepareRecordTransition(repo, {
  ...
  const temporary = mkdtempSync(path.join(tmpdir(), 'protocol-record-index-'));

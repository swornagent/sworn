...
const LEGACY_RECORD_ROOT = '.protocol/releases';
...
export function unsafeProductBaseEvidence(state) {
  ...
  if (!evidence) {
    throw new TypeError('product-base evidence requires an exact Protocol state snapshot');
  }
  return evidence;
}
...
export class ProtocolStateError extends Error {
  constructor(code, message, cause) {
    ...
    this.name = 'ProtocolStateError'; this.code = code;
  }
}
function fail(code, message, cause) { throw new ProtocolStateError(code, message, cause); }
...
function validateSlice(
  ...
  for (const entry of entries) {
    ...
    if (
      ...
      || !['implementer', 'lead', 'verifier'].includes(receipt.role)
    ) {
    ...
    if (receipt.role === 'implementer' && receipt.result === 'designed') {
      ...
      const retry = sameLineage
        ...
        && (
          (bound.role === 'lead' && bound.result === 'revise')
      ...
      const staleReviewRetry = sameLineage
        ...
        && (
          ...
          || (bound.role === 'lead' && bound.result === 'proceed')
...
function deriveSlice(location, history, current, approval, planByOID) {
  ...
  if (
    ...
    && (
      (currentReceipt.receipt.role === 'lead'
  ...
  const status = {
    'implementer/designed': ['design', 'ready', 'lead', 'none'],
    'implementer/candidate': ['verify', 'ready', 'verifier', 'none'],
    'lead/proceed': ['implement', 'ready', 'implementer', 'proceed'],
    'lead/revise': ['design', 'ready', 'implementer', 'revise'],
    'lead/escalate': ['design', 'blocked', 'planner', 'escalate'],
  ...
  if (['lead/revise', 'verifier/fail'].includes(key)) state.attempt += 1;
...
export function readProtocolState(
  ...
  if (typeof release !== 'string' || typeof captureRefs !== 'function') {
    throw new TypeError('readProtocolState requires a release string and captureRefs function');
  }
...
export function isProtocolStateError(error) {
  return error instanceof ProtocolStateError

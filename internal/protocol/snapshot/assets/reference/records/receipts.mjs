...
export const PLAN_VERSION = 'protocol.plan/v2';
export const RECEIPT_TRAILER = 'Protocol-Receipt: ';
export const DETAIL_BEGIN = 'Protocol-Detail-Begin';
export const DETAIL_END = 'Protocol-Detail-End';
...
const RESULT_BY_ROLE = Object.freeze({
  ...
  lead: new Set(['proceed', 'revise', 'escalate']),
  ...
  for (const scopedPath of result.include) {
    if (isReservedRecordPath(scopedPath)) {
      fail(
        ...
        `${label}.include cannot name reserved Protocol records at ${scopedPath}`,
      );
    }
  }
  return result;
...
export function parsePlanBytes(value) {
  ...
  const open = Buffer.from('```protocol-plan-v2\n');
  ...
  if (closeAt === -1 || input.indexOf(close, closeAt + close.length) !== -1) {
    fail('INVALID_PLAN_FENCE', 'plan must contain one closed protocol-plan-v2 block');
  }
...
export function validateReceipt(value) {
  ...
  } else if (role === 'lead' || (role === 'implementer' && value.result === 'designed')) {

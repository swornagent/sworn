...
export class ProtocolActionError extends Error {
  constructor(code, message, cause) {
    super(message, cause ? { cause } : undefined);
    this.name = 'ProtocolActionError';
    this.code = code;
  }
}

function fail(code, message, cause) {
  throw new ProtocolActionError(code, message, cause);
}
...
function receiptResult(action, changed, details) {
  return frozen({
    kind: 'protocol.action-result/v2',
    action,
    changed,
    ...details,
  });
...
export function createProtocolActions(options) {
  ...
  function stateFor(release) {
    return readProtocolState(repo, release);
  }
  ...
  function appendReceipt(rawOptions) {
    ...
    if (sliceID !== null) {
      ...
      } else if (role === 'lead' && ['proceed', 'revise', 'escalate'].includes(result)) {
        if (slice.next_role !== 'lead') {
          fail('ROLE_NOT_ELIGIBLE', `${sliceID} does not currently need Lead review`);
        }

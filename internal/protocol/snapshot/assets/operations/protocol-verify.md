...
- Work identity, approved plan, exact candidate, and slice Lead decision.
...
access, missing approval, a stale Lead decision, candidate movement after
...
`PASS` hands to `protocol-merge`; `FAIL` returns the same stable slice to
`protocol-implement`; `BLOCKED` hands to `protocol-plan`. The engine chooses

---
operation: protocol-design-review
...
- Relevant repository facts and the Lead invocation.

## Authority

Review only this design attempt. The Lead must differ from its producer and
...
contract and authority stay unchanged. The Lead never widens its own remit.

An exact recorded delegation may let the Lead authorize or revise an exact
...
bindings and the Lead invocation under technical details. Do not write the
Lead receipt.
...
`PROCEED` returns to `protocol-implement` for implementation or bounded repair;

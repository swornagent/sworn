...
2. After all tracks pass, assemble the latest target with those exact products
   and stop for fresh `protocol-verify` assembly. A later target advance rebuilds
...
unreconciled effect. A stale snapshot needs a rescan, not a Protocol verdict.

## Next handoff

Completed track composition waits for other tracks or assembly verification.
An exact release merge finishes Protocol; deployment happens elsewhere.

---
operation: protocol-implement
version: protocol.operation/v2
---

## Purpose

Explain an approach or build one ready slice without doing the Lead's or
Verifier's job.
...
- The Lead decision and prior Verifier decision when present.
...
1. Before design, use the current release plan and the exact base the engine
   ...
   history. Batch independent reads, searches, and inspection probes into one
   turn. Return a design TL;DR that maps every acceptance identifier to the
...
and Lead handoff. Implementation output includes checks, evidence,
...
operational failure without inventing a Protocol outcome.

## Next handoff

Send a design to `protocol-design-review` and candidate evidence to
`protocol-verify`. The engine applies the Protocol's direct-repair rule and
assurance policy.

# Protocol Conformance 1.0

Following Protocol means the system actually protects the five handoffs. Merely
...
2. bind each plan revision and compact responsibility receipt to exact immutable
   objects;
3. keep Planner, Implementer, Lead, Verifier, Merge, and external-authorizer
   authority distinct;
4. require an applicable Lead `PROCEED` before implementation;
...
The configured records root is structurally reserved for Protocol metadata:
product code, build, test, package, deploy, hooks, and runtime MUST NOT read or
depend on it; only Protocol's record writer may modify it; product identity ignores
...
approval, uses a distinct Lead, starts an independent Verifier thread fresh,
...
- timeout, cancellation, crash, malformed output, and retry exhaustion never
  manufacture a Protocol outcome;

# Protocol autonomous-engine conformance adapter

Version: `protocol.engine-conformance/v1`

This small command-line contract lets Protocol ask a real engine to run one
...
```json
{
  "schema_version": "protocol.engine-conformance-request/v1",
  "invocation_id": "unique stable string",
  "case_id": "manifest autonomous case id",
  "protocol_version": "<installed Protocol VERSION>",
...
and process boundaries. It must not replace those boundaries with Protocol
reference helpers or an in-process simulation. Retried or reconciled runtime
failures produce engine evidence only; they do not create a Protocol receipt
...
```json
{
  "schema_version": "protocol.engine-conformance-result/v1",

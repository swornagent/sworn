...
const (
	PlanVersion     = "protocol.plan/v2"
	ManifestVersion = "sworn.release-manifest/v1"
	planOpen        = "```protocol-plan-v2\n"
	planClose       = "\n```\n"
...
// Plan is an immutable admission of exact protocol.plan/v2 bytes.
type Plan struct {
...
// ParsePlan admits either the legacy protocol.plan/v2 fence (unchanged) or the
...
func ParsePlan(raw []byte) (Plan, error) {
	...
	case bytes.HasPrefix(raw, []byte(planOpen)):
		return parseFencedPlan(raw, planOpen, planClose, "protocol-plan-v2", validatePlanMetadata)
...
// Legacy protocol.plan/v2 plans carry their slice bodies inline and have no
...
// the legacy inline protocol.plan/v2 shape; validateManifestSlice implements it
...
// validateSlice parses one legacy protocol.plan/v2 inline slice entry: the
	...
	for _, scopedPath := range includes {
		if isReservedRecordPath(scopedPath) {
			return Slice{}, "", recordFail(
				...
				label+".scope.include cannot name reserved Protocol records at "+scopedPath,
			)
		}
	}
...
func parseWaivers(value any, label string) ([]ScopeWaiver, error) {
	...
	for index, raw := range items {
		...
		if isReservedRecordPath(pkg) {
			return nil, recordFail("RESERVED_RECORD_ROOT", itemLabel+".package cannot name reserved Protocol records at "+pkg)
		}
		if seen[pkg] {
...
func validateManifestSlice(value any, trackID, label string) (Slice, string, error) {
	...
	if isReservedRecordPath(contractPath) {
		return Slice{}, "", recordFail(
			"RESERVED_RECORD_ROOT",
			label+".contract_path cannot name reserved Protocol records at "+contractPath,
		)
	}
	...
	for _, touchpoint := range touchpoints {
		if isReservedRecordPath(touchpoint) {
			return Slice{}, "", recordFail(
				"RESERVED_RECORD_ROOT",
				label+".touchpoints cannot name reserved Protocol records at "+touchpoint,
			)
		}
	}

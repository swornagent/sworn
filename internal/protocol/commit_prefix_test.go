...
// prefix never strands existing history: protocol( and sworn( subjects remain
...
func TestHistoricalPrefixesStillParseAndAreDetected(t *testing.T) {
	...
	for _, subject := range []string{
		"protocol(rel): plan revision 1",
		"sworn(rel): plan revision 1",
		"protocol(rel/slice): implementer designed",
	...
	for _, subject := range []string{"protocol(rel): x", "sworn(rel): x", "Protocol exact ", "Protocol engine-owned "} {
...
func historicalEngineSubject(message string) bool {
	return strings.HasPrefix(message, "protocol(") || strings.HasPrefix(message, "sworn(") ||
		strings.HasPrefix(message, "Protocol exact ") || strings.HasPrefix(message, "Protocol engine-owned ")
}

...
// Identity is fixed commit attribution supplied by the policy owner.
type Identity struct {
	Name  string
	Email string
}

// BlobChange is one regular-file update or deletion. Delete ignores Bytes.
...
// RecordRequest describes a deterministic single-parent tree/commit.
...
// PreparedCommit is an immutable object result; no ref has moved.
...
// PrepareRecord writes immutable objects for a deterministic single-parent
// commit without advancing a ref.
...
// CompositionMode records the only mechanically possible composition.
...
// CompositionRequest contains fixed policy-derived metadata; callers cannot
// choose a merge mode.
...
// PreparedComposition is immutable and does not move a ref.
...
// PrepareComposition returns fast-forward when possible, rejects containment,
// and otherwise computes Git's deterministic ordered two-parent merge tree.
...
func (r *Repository) AtomicUpdateRefs(operations []RefOperation) error {
	...
	input.WriteByte(0)
	if _, err := r.run(input.Bytes(), nil, "update-ref", "--stdin", "-z"); err != nil {

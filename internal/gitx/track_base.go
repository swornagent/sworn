...
// chain used to prepare one consumer track. ConsumerBefore nil means the
// consumer ref is absent and the exact release head is the composition seed.
...
func (w *Workspaces) validateTrackBaseRequest(
	...
	seed := request.ReleaseHead
	if request.ConsumerBefore != nil {
		seed = *request.ConsumerBefore
	}
	return consumerRef, seed, nil
}
...
func (w *Workspaces) expectedTrackBase(
	...
	for _, input := range request.Inputs {
		...
		prepared, err := w.repository.PrepareComposition(CompositionRequest{
			Expected:         base,

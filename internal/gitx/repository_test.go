...
func nextRecord(t *testing.T, repository *Repository, parent OID, name string) OID {
	...
	record, err := repository.PrepareRecord(RecordRequest{
		Parent: parent, Changes: []BlobChange{{Path: name + ".txt", Bytes: []byte(name + "\n")}},
...
func TestRC3ExactCreateUpdateVerifyReconcileAndRetry(t *testing.T) {
	...
	for _, format := range []ObjectFormat{SHA1, SHA256} {
		...
		t.Run(string(format), func(t *testing.T) {
			...
			before, err := repository.ProductTreeIdentity(base)
			if err != nil {
				t.Fatal(err)
			}
			after, err := repository.ProductTreeIdentity(first.Commit)
			if err != nil {
				t.Fatal(err)
			}
			if before != after {
...
func TestCapturedReadsAndDeterministicComposition(t *testing.T) {
	for _, format := range []ObjectFormat{SHA1, SHA256} {
		...
		t.Run(string(format), func(t *testing.T) {
			...
			product, err := repository.ProductTreeIdentity(first.Commit)
			if err != nil {
				t.Fatal(err)
			}

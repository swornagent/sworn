...
func TestClaudeCLI_NormalDispatch(t *testing.T) {
	...
	text, cost, err := d.Verify(context.Background(), "you are a verifier", "check this")
	if err != nil {
...
func TestClaudeCLI_MissingBinary(t *testing.T) {
	...
	_, _, err := d.Verify(context.Background(), "s", "u")
	if err == nil {
...
func TestClaudeCLI_FromEnvIntegration(t *testing.T) {
	...
	text, cost, err := verifier.Verify(context.Background(), "sys", "payload")
	if err != nil {
		t.Fatalf("Verify() error: %v", err)
	}
	if text != "PASS" {
...
func TestClaudeCLI_ModelPassthrough(t *testing.T) {
	...
	_, _, err := d.Verify(context.Background(), "sys", "usr")
	if err != nil {
		t.Fatalf("Verify() error: %v", err)
	}

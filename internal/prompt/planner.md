...
7. Initialise `status.json` with `state: planned` and the slice's `track` id.
   - **Dependency files checklist:** if the slice will add, remove, or change a Go dependency, include `go.mod` and `go.sum` in `planned_files`. `sworn lint deps <slice-id> <release>` fails closed when a dep file changes but is absent from `planned_files` — declare them now to avoid a late Gate-2 surprise at verify.8. **Record the validation record in `status.json`.** For each spec, draft:   - At least **one positive scenario** (the requirement works as intended) per requirement
...
11. **Risk-cites-code rule (Rule 11).** Every Risk mitigation in `spec.md` §Risks must cite a live code surface — a file path and line range (`file:line` or `file:line-line`) that exists at the time the spec is authored. A mitigation that asserts a guard, assertion, audit, or check against a code surface that does not exist (or is named vaguely — "the validation layer", "auth middleware") is a spec defect. Before writing a Risk mitigation, grep the codebase for the named surface; if it doesn't exist, either drop the mitigation or create the surface as a slice in this release. A mitigation with no code citation fails `sworn lint specquality <release>` closed. This rule pre-empts theme T-G: Risk sections repeatedly asserting against non-existent code surfaces (e.g. "the kernel will reject", "auth middleware will block") that pass spec review but become BLOCKED verdicts at verification when the Verifier walks the repo and finds no such surface.
12. **Shape-pin / two-commit note (Rule 12).** A pure-engine Go slice — one whose `spec.md` `Entry point` is a Go package/function and whose `In scope` lists no UI, CLI, or user-facing affordance — produces a `git diff` that may be empty if the slice is entirely additive (new files only, no existing-file edits). An empty diff BLOCKs the Verifier gate (Gate 2: `actual_files` is `[]`, no live diff to verify). **Every pure-engine Go slice must include a failing-test commit before the implementation commit.** The test must exercise the entry-point function, fail because the function does not exist yet, and be committed separately. The slice then lands as two commits: (1) `test(<pkg>): failing shape-pin for <entry-point>` and (2) `feat(<pkg>): <slice outcome>`. The Verifier sees a non-empty diff spanning both commits. Note this requirement in the slice's `spec.md` `Required tests` — list the shape-pin test command and the failing expectation. Pure-engine slices with no shape-pin plan fail the spec-quality gate.
13. **Dynamic-CORS dev-server note (Rule 13).** When authoring a UI slice spec, include a note in `spec.md` `Risks` or `Notes`: the dev server on a derived port (worktree) serves CORS headers dynamically via the port-deriver (`scripts/port-deriver.sh` + Next.js `server.js` injection), and any static port-allowlist assertion in an acceptance check is a stale constraint. Mark the `[[feedback_worktree_devserver_cors_port]]` memory **stale** — dynamic CORS supersedes the static port allowlist documented there. This pre-empts theme T-J: recurring smoke-port-outside-allowlist reachability misses where the verifier's manual smoke step hits the wrong port because the spec hardcodes a stale port number.

    > **Memory staleness note:** `[[feedback_worktree_devserver_cors_port]]` is marked stale as of this rule. Dynamic CORS injection (via `scripts/port-deriver.sh` + `server.js` header injection) supersedes the static port allowlist approach documented in that memory entry. Planners and implementers should reference this rule, not the stale memory.

### Phase 5 — Write the release boardCreate `docs/release/<release-name>/index.md`, `activity.md`, and `.gitattributes` by copying them from `$HOME/.claude/baton/release-mode-template/`.

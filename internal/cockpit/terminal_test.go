...
func TestRenderTerminalRendersCheckpoints(t *testing.T) {
	...
	output := RenderTerminal(snapshot)
	if !strings.Contains(output, "CHECKPOINTS count=3") {
		t.Fatalf("expected CHECKPOINTS section in terminal output:\n%s", output)
	}
	if !strings.Contains(output, "status=\"saved\"") || !strings.Contains(output, "status=\"restored\"") || !strings.Contains(output, "status=\"fenced\"") {
		t.Fatalf("expected all 3 checkpoint statuses in terminal output:\n%s", output)
	}

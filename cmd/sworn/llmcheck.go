...
//
// Each check calls a configured LLM (default: SWORN_MODEL env var or
// --model flag) with a structured prompt, parses the JSON verdict, and
...
func cmdLLMCheck(args []string) int {
	...
	modelID := fs.String("model", "", "model ID in provider/model format (default: $SWORN_MODEL)")
	...
	if mid == "" {
		fmt.Fprintln(os.Stderr, "sworn llm-check: no model configured (set --model or $SWORN_MODEL)")
		return 2
	}

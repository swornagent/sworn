...
func boardGitOutput(t *testing.T, dir string, args ...string) string {
	t.Helper()
	cmd := exec.Command("git", args...)
	cmd.Dir = dir
	out, err := cmd.CombinedOutput()
...
func snapshotRepo(t *testing.T, repoDir string) repoSnapshot {
	t.Helper()
	refs := strings.Fields(boardGitOutput(t, repoDir, "for-each-ref", "--format=%(refname)"))
	sort.Strings(refs)
	cwd, err := os.Getwd()
	if err != nil {
		t.Fatal(err)
	...
	return repoSnapshot{
		Head:   strings.TrimSpace(boardGitOutput(t, repoDir, "rev-parse", "HEAD")),
		Branch: strings.TrimSpace(boardGitOutput(t, repoDir, "branch", "--show-current")),
		Refs:   strings.Join(refs, "\n"),
		Status: boardGitOutput(t, repoDir, "status", "--porcelain"),
		CWD:    cwd,
	}
}
...
func TestBoardCLIAllRefsCatalogSourceRef(t *testing.T) {
	...
	topology := strings.TrimSpace(boardGitOutput(t, repoDir, "rev-parse", "HEAD"))
	runGit(t, repoDir, "reset", "--hard", "HEAD^")
	...
	for _, scope := range []struct {
		...
		for _, defect := range []string{"missing", "malformed", "identity-mismatch"} {
			t.Run(scope.name+"-"+defect, func(t *testing.T) {
				...
				runGit(t, repoDir, "add", "README.md")
				runGit(t, repoDir, "commit", "-m", "consumer head")
				base := strings.TrimSpace(boardGitOutput(t, repoDir, "rev-parse", "HEAD"))
				...
				lower := strings.TrimSpace(boardGitOutput(t, repoDir, "rev-parse", "HEAD"))
				...
				if defect != "missing" {
					...
					canonical = strings.TrimSpace(boardGitOutput(t, repoDir, "rev-parse", "HEAD"))
					runGit(t, repoDir, "reset", "--hard", base)
				}

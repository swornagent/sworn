...
- [ ] **User-initiated pause/resume**: a cooperative pause signal (e.g. `sworn pause <release>`) is honoured at the next poll — the worker finishes the in-flight dispatch, persists committed state, and stops; `sworn resume <release>` (or re-running `sworn run`) continues from committed state. The pause/resume control is part of the engine (not the surface) so it is invokable identically from CLI, TUI, and the MCP run-control tools — see `docs/decisions/2026-06-24-sworn-orchestration-surfaces-and-subscription-drivers.md`.

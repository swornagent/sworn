...
## 2026-07-17 — remediation complete; ready for fresh verification

- Corrected `DiscoverCatalog` so it batch-reads direct topology artifacts, validates them before source-ref election, and excludes unreadable or identity-mismatched noncanonical candidates. A malformed noncanonical candidate can no longer abort an unrelated valid catalog or displace a lower-priority valid candidate. Canonical release-worktree refs remain authoritative and retain the existing missing/malformed/identity-mismatch fail-closed behavior.
- Added `TestBoardCLIAllRefsCatalogSkipsInvalidNoncanonicalTopology`, a compiled-CLI fixture with a lexically earlier malformed bare-string board, a later valid noncanonical candidate for the same release, and an invalid-only historical release. It proves valid fallback selection, invalid-only omission, and named-query reachability; the prior canonical-skew fixture remains green.

...
- **Rule 2 deferral (launch-dir dirty files)**: the primary repo at `/home/user/projects/sworn` (branch `release/v0.1.0`) carries uncommitted modifications to `internal/mcp/catalog.go`, `context.go`, `tools_ops.go`, `tools_plan.go` (~151+/209-) — S04-shaped work apparently done in the wrong tree by an earlier session (why it exists). This session did not touch it; work happened only in the T3-mcp track worktree. Tracking: surfaced in the implementer's session-end output for a human decision (keep/discard). Acknowledgement: same output.

...
7. [escalate] AC-06 reachability-artefact substitute needs Coach sign-off
   ...
   What to ask the Coach: is the Go-level no-op-`RunSliceFn` substitute acceptable as AC-06's reachability artefact, or is the literal CLI invocation required (accepting the side effects on T1-T3's live worktrees)? Option (a) (substitute) prioritises isolation from other in-flight tracks; option (b) (literal CLI) prioritises literal fidelity to the spec's stated reachability wording. This is not determinable from repo state alone.

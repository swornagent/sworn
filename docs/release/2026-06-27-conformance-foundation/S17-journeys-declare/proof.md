...
- Spec planned touchpoints listed only `.sworn/journeys.json`; implementation also modified `internal/journey/journey.go`, `internal/journey/journey_test.go`, and `internal/baton/schemas/journeys-v1.json`. Reason: AC4 requires each journey to declare a `no_mock_boundary` field that did not exist on the `Journey` struct or in the journeys-v1 schema before this slice. `journey.go`: added `NoMockBoundary` string field. `journeys-v1.json`: added `no_mock_boundary` property so the artefact validates (AC2 + Pin 2). `journey_test.go`: `TestCheck_S17Journeys` asserts `NoMockBoundary != ""` on each journey (Pin 3 from design review). These changes are the minimum required to deliver AC4 and were implicit in the spec outcome but not enumerated in planned touchpoints.

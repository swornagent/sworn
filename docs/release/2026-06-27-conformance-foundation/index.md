...
- Verified (awaiting merge): 11 (S10-agentic-chat-anthropic, S13-schema-embed-validate, S14-board-json, S15-spec-proof-records, S16-journeys-attestations-align, S18-orchestrator-formalized, S19-captain-split, S20-role-revendor, S21-sworn-run-task, S22-pin-bump, S23-version-centralise-doctor)- Failed verification: 0
- Deferred: 0
- Shipped: 0
...
|---|---|---|---|
| J1 | keyless-full-loop | entitlement / credits | declared (failed_verification) |
| J2 | loop-verifier-negative | loop-verifier (real gate, not stateless judge) | declared (failed_verification) |
| J3 | ship-a-release (surface-seam) | Driver 1/2/3 end-to-end, real board + real gates | declared (failed_verification) |
...
### 2026-06-28 — slice `S17-journeys-declare` FAILED verification

- **Actor**: verifier (/verify-slice, fresh context)
- **Verdict**: FAIL. Gate 2 — 3 code files (`internal/journey/journey.go`, `internal/journey/journey_test.go`, `internal/baton/schemas/journeys-v1.json`) modified but not in spec touchpoints and not explained in proof.json divergence. All other gates (1, 3, 4, 5, 6, 7) pass. Remediation: add missing files to proof.json divergence explanation.

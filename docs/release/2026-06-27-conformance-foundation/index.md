...
| `S09-error-kind-consumption` | T2 | KindAuth, KindCredits, and other terminal Error{Kind}s halt the loop immediately without retry; the factory sentinel is correctly named | verified | [spec](./S09-error-kind-consumption/spec.md) | [proof](./S09-error-kind-consumption/proof.md) |
...
- Verified (awaiting merge): 2 (S08-capability-descriptor, S09-error-kind-consumption)
...
- 2026-07-25 — S09-error-kind-consumption **verified** (PASS). All 7 gates clear. Terminal-error halt (KindAuth/KindCredits) placed before triage path; `ErrDriverNotRegistered` → `ErrDriverNotImplemented` rename consistent across 18 references; `IsTerminal()` correctly returns true only for KindAuth/Credits. Next: `/implement-slice S10-agentic-chat-anthropic`.

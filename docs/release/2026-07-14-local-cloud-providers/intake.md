...
- **Codex CLI verifier and structured-schema adaptation**: `sworn llm-check` cannot currently resolve `codex/` through `model.FromEnv`, and Codex CLI 0.144.3 rejects the canonical `llm-check-report-v1` response schema because its top-level `allOf` is unsupported by the Responses structured-output subset. **Why deferred**: this is a cross-cutting verifier-runtime concern, not endpoint declaration or provider dialect scope; folding it into this release would couple planning-tool repair to the operator outcome. **Tracking**: [sworn#19](https://github.com/swornagent/sworn/issues/19), updated with the live finding and required local full-schema/post-verdict validation. **Acknowledged**: repository owner, 2026-07-14, explicitly requested durable capture so Sworn can support this path.
...
### 2026-07-14 — Capture Codex CLI ambiguity-gate compatibility separately

- **Context**: The configured Anthropic verifier could not run because no API key was present and organization-level Claude subscription access was disabled. A human-authorized direct `codex --yolo exec --ephemeral` fallback reached Codex, but `--output-schema internal/baton/schemas/llm-check-report-v1.json` failed with `invalid_json_schema` because Codex structured output does not permit the schema's top-level `allOf`. A temporary subset schema succeeded when the canonical derived-verdict invariant was enforced locally afterward.
- **Options considered**: silently bypass the ambiguity gate; add Codex verifier/schema-adapter work to this endpoint release; capture the product gap on the existing Codex driver issue and use direct fresh-context Codex checks for this planning session.

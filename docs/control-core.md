...
This milestone does not validate Baton plans, cryptographically resolve
authority, run checks or coding agents, inspect Git, execute effects, or mutate a
target. `sworn board` is the only new CLI command and opens the control store in
read-only mode. The reducer's activation and build-dispatch transitions exist to
prove the command/effect boundary; they are unreachable from the CLI.

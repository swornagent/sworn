...
func TestRealBinaryDelegatedLeadProceedInstallsRC14AndContinuesSerially(t *testing.T) {
	...
	provider := &journeyProvider{
		...
		leadPlanOutcomes: []driver.DecisionOutcome{driver.DecisionRevise, driver.DecisionProceed},
	...
	_, shapeDigest, err := swornruntime.LeadPlanStructuralProjection(plan)
	if err != nil {
		t.Fatal(err)
	}
	values, err := swornruntime.LeadPlanValueProjection(plan)
	if err != nil {
		t.Fatal(err)
	}
	revisedValues, err := swornruntime.LeadPlanValueProjection(revisedPlan)
	if err != nil {
		t.Fatal(err)
	}
	...
	fieldRules := make([]swornruntime.LeadFieldRule, 0, len(pointers))
	var delta swornruntime.LeadDeltaOperation
	for _, pointer := range pointers {
		...
		if revisedValues[pointer] != values[pointer] {
			...
			delta = swornruntime.LeadDeltaOperation{Operation: "replace", JSONPointer: pointer, FromDigest: values[pointer], ToDigest: revisedValues[pointer]}
		}
		fieldRules = append(fieldRules, swornruntime.LeadFieldRule{
	...
	envelope := swornruntime.LeadDelegation{
		SchemaVersion: swornruntime.LeadDelegationVersion,
		...
		ReleaseLineageAnchor: swornruntime.LeadLineageAnchor{State: "absent"},
		TargetRef:            "refs/heads/main", TargetHead: targetHead, DelegationEpoch: 1,
		DelegateRole: "lead", Responsibility: swornruntime.LeadPlanReviewResponsibility,
		DecisionRules: []swornruntime.LeadDecisionRule{
		...
		Limits: swornruntime.LeadDelegationLimits{
		...
		PlanRules: swornruntime.LeadPlanPolicy{
			SchemaVersion:  swornruntime.LeadPlanPolicyVersion,
			AuthorityClass: "ordinary_delivery", InitialShapeDigest: shapeDigest,
			FieldRules: fieldRules,
			DeltaRules: swornruntime.LeadDeltaRules{MaximumOperations: 1, AllowedOperations: []swornruntime.LeadDeltaOperation{delta}},
		},
	}
	envelopeBytes, err := swornruntime.CanonicalLeadDelegation(envelope)
	if err != nil {
		t.Fatal(err)
	}
	...
	if counts["command:attention.answer"] != 2 ||
		counts["command:lead_decision"] != 2 || counts["command:planner_continuation"] != 1 || counts["command:approval"] != 1 ||
		counts["effect:approval.admit"] != 1 || counts["effect:protocol.install"] != 1 ||
		counts["effect:planner.continue"] != 1 || counts["event:lead_plan_decided"] != 2 {
	...
	for _, effect := range snapshot.Effects {
		...
		if decodeErr == nil && submission.Responsibility == driver.LeadPlanReview {
			leadInvocations[submission.InvocationID] = true
		}
	}
	if len(leadInvocations) != 2 {
		t.Fatalf("replacement Lead invocations = %#v", leadInvocations)
	}
